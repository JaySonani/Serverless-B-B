...
def confirm_intent(slots, message):
    return {
        'dialogAction': {
            'type': 'ConfirmIntent',
            'intentName': BookTour,
...
def response_card_close(message, NAVIGATION_URL):
    response = {
        "dialogAction": {
            "type": "Close",
...
def get_navigation_link(webpage):
    ...
    LOGIN_URL = 'https://storage.googleapis.com/siteimages/login.png'
    signup = ['signup', 'sign up', 'sign-up', 'register']
    SIGNUP_URL = 'https://storage.googleapis.com/siteimages/signup.png'
    tour = ['tour', 'book tour', 'tour booking']
    TOUR_URL = 'https://storage.googleapis.com/siteimages/tour.png'
    room = ['room', 'room booking', 'book room']
    ROOM_URL = 'https://storage.googleapis.com/siteimages/room.png'
    meal = ['meal ordering', 'meal', 'food order', 'order food']
    MEAL_URL = 'https://storage.googleapis.com/siteimages/meal.png'
...
def validate_meal_order(slots):
    ...
    if meal_quantity and isvalid_meal_quantity(meal_quantity):
        return build_validation_result(
            False,
            'mealQuantity',
            '{} is invalid value for number of meals. Kindly provide numeric value for number of meals'.format(
                meal_quantity)
        )
...
def validate_book_room(slots):
    ...
    if room_quantity and isvalid_room_quantity(room_quantity):
        return build_validation_result(
            False,
            'room_quantity',
            '{} is invalid value for number of rooms. Kindly provide numeric value for number of rooms'.format(
                room_quantity)
        )
...
def navigation(intent_request):
    ...
    if NAVIGATION_URL:
        fulfillmentState = 'Fulfilled'
        message = f'Click on the menu item as displayed in below image to navigate to {webpage} page'
        return response_card_close(message, NAVIGATION_URL)
    else:
...
def get_available_rooms(intent_request):
    ...
    if res.status == 200:
        ...
        for key, value in response.items():
            availability += key + " => "
            for room_type, qty in value.items():
                availability += room_type.replace('count_', '') + ": " + str(qty) + " "
